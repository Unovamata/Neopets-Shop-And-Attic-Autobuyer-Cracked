...
function getSHOULD_USE_NEON(callback) {
    chrome.storage.local.get(['SHOULD_USE_NEON'], function(result) {
        const shouldUseNeon = result.SHOULD_USE_NEON;
        if (typeof callback === 'function') {
        callback(shouldUseNeon);
        }
    });
}

getSHOULD_USE_NEON(function (isNeonAvailable){
    //Checking if the Neon extension is true in the extension options.
    if(!isNeonAvailable) return;
